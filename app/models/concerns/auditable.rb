...
module Auditable
  ...
  TRACKED_FIELDS = {
    ...
    "Inquiry"                => %w[category status order_id is_visible_to_agent is_visible_to_customer],
    ...
    # R6-8: ファイル管理基盤。ファイル本体（Active Storage blob）はAuditLogの対象外とし、
    # 「どのOrderに・どんな分類のファイルが・社内限定/顧客公開のどちらで」という構造情報のみ追う。
    "OrderAttachment" => %w[order_id file_type is_visible_to_customer]
  }.freeze
