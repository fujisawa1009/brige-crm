...
| Q-21 | 通知の受信者マトリクス | 一部決定済み（2026-08-19・v5） | R4 実装済み / R6 着手前チェック残 | E3(差戻し)=営業担当者・社内実務担当者（顧客へは通知しない）／E5(契約確定)=顧客・営業担当者・社内実務担当者／E6(決済失敗)=**通知不要**（Web商談でオンライン中に画面表示のため）で決定済み。残る未決: E7(遅延)・E8(自動キャンセル)・E11(メンション)・E12(一斉通達)は R6 着手前チェック |
