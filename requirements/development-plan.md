...
| ~~Q-F~~ | 商材が今後増えるか（増えるなら納品日は別テーブル化） | ✅ **決定を更新（2026-08-19・v5。旧D-4=2026-07-26 を上書き）** | R5前確定済み（04 R5着手前チェックリスト） | 旧D-4「増える見込み→別テーブル化」から**現状維持（`work_completed_at` 単一カラムのまま。別テーブル化しない）へ変更**。CEOが両案を比較のうえ現状維持を再確認・優先すると決定（2026-08-19） |
