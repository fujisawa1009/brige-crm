...
## 0-0. 【最上位】CEO決定 2026-08-20: 画面表示は原則「ステータス」（D-8 の表示ルールを上書き）

> **この節は D-8（2026-07-26 案A）より優先する。以降の §0〜§5 の「表示名」に関する記述は、本節の適用ルールを
> 満たす範囲でのみ有効。巻き戻し禁止**（過去に決定の記録漏れで実装が巻き戻る事故が発生しているため、
> 次の担当者は本節を最初に読むこと）。

- **決定（2026-08-20 CEO・画面目視確認による）**: 管理画面の項目ラベルは、原則として**単に「ステータス」**と表示する。
  D-8 が定めた「申込ステータス」「案件ステータス」という表示名は、**下記の例外を除いて使わない**。
- **決定の射程**: 変わるのは**画面上の表示ラベルだけ**。用語体系（案件／申込／契約の3語）・テーブル名・
  カラム名・コード内コメント・設計文書中の用語は **D-8 のまま変更しない**（「顧客ステータス」は引き続き使用禁止語）。

### 適用ルール（2026-08-20）

| 区分 | 表示 | 例 |
|---|---|---|
| その画面の**主エンティティ自身**のステータスを指す場合 | **「ステータス」** | 顧客一覧の絞り込み・表の列見出し、顧客フォーム、案件一覧の絞り込み・列見出し |
| **顧客と案件のステータスが同一画面に並ぶ**場合 | 修飾付きを維持 | 顧客詳細（基本情報の申込ステータス＋案件セクションの案件ステータスバッジ） |
| **案件と契約のステータスが同一画面に並ぶ**場合 | 修飾付きを維持 | 案件詳細（案件ステータス／契約ステータス）、案件フォーム（契約ステータスの注記が同居） |
| **案件一覧の絞り込みに顧客のステータスが同居する場合** | 案件自身は「ステータス」／顧客側は**「顧客ステータス」**（下記 追記2026-08-20 を参照） | 案件一覧の絞り込み |
| **マスタ管理画面の画面名**・ナビゲーション | 修飾付きを維持 | 「申込ステータス」「案件ステータス」「契約ステータス」「問い合わせステータス」 |
| **CSVエクスポートのヘッダ** | 修飾付きを維持（区別が必要なため） | 現行実装のヘッダは英字カラム名（`status`）のため該当箇所なし。日本語ヘッダを導入する際は修飾付きにすること |

### 実装状況

- ✅ 2026-08-20 実装済み。適用: `admin/customers/index.html.erb`（絞り込みラベル・th）、`admin/customers/_form.html.erb`、
  `admin/orders/index.html.erb`（絞り込みラベル・th）。
- 併せて **D-8 の使用禁止語「顧客ステータス」の残存2箇所**を「申込ステータス」へ是正:
  `app/helpers/admin_nav_helper.rb`（サイドナビ）・`app/views/admin/system_settings/show.html.erb`（マスタ導線）。
- 維持（修飾付きのまま）: `admin/customers/show.html.erb`（申込ステータス）、`admin/orders/show.html.erb`（案件ステータス／契約ステータス）、
  `admin/orders/_form.html.erb`（案件ステータス）、`admin/{customer_statuses,order_statuses,contract_statuses,inquiry_statuses}/*`（マスタ画面名）。

### 追記 2026-08-20: 案件一覧の絞り込みラベルに「顧客ステータス」を用いる（D-8 の当該制約を上書き）

> **巻き戻し禁止。** D-8（2026-07-26 案A）は「顧客ステータス」を**使用禁止語**としていたが、
> 下記のケースに限り **CEOが明示的に「顧客ステータス」を指定した**ため、その制約を上書きする。
> 次の担当者は「禁止語だから」という理由でこのラベルを書き換えないこと。

- **決定（2026-08-20 CEO・案件一覧の検索条件12件の指定に際して）**: 案件一覧（`/admin/orders`）の
  絞り込みに置く「顧客側のステータス」の項目ラベルは **「顧客ステータス」** とする。
- **理由**: 案件一覧には案件自身のステータス（ラベル「ステータス」）と顧客のステータスが**同一画面に並ぶ**ため、
  区別が必要。§0-0 の適用ルール表でいう「顧客と案件のステータスが同一画面に並ぶ場合＝修飾付きを維持」に該当する。
- **射程**: 変わるのは**この画面のこの項目ラベルだけ**。
  - **顧客一覧（`/admin/customers`）のラベルは「ステータス」のまま**（変更しない。その画面の主エンティティ自身のステータスであるため）。
  - 案件一覧の**案件自身**のステータスも「ステータス」のまま（CEO指定どおり）。
  - 用語体系・テーブル名・カラム名・コード内コメント・設計文書中の用語は D-8 のまま変更しない。
    ここで指すのは実装上 `customers.status`（`customer_statuses` マスタ＝申込8値）であり、
    設計文書中では引き続き「申込ステータス」と呼ぶ。
- **実装**: `app/views/admin/orders/index.html.erb`（絞り込みラベル・`name="customer_status"`）、
  `app/services/order_search.rb`（パラメータ名 `customer_status`。案件自身の `status` と明確に分ける）。
...
| 2026-08-20 | **CEO決定: 画面表示は原則「ステータス」（D-8 の表示ルールを上書き）**。§0-0 を追加（適用ルール・例外・実装状況）。用語体系・テーブル名・文書用語は D-8 のまま |
