...
| **R-13（2026-08-19追加。出典: 2026-08-18浅賀MTG）** | **ネットムーブとの決済会社契約の巻き直しが未了**（社内SSSでの契約再締結が必要とMTGで判明）。契約が完了しない限り本番接続審査・決済開通（P5-14/R8-D-1）そのものが進められない。**⚠️ `netmove-card-migration.md` §0「引き継がれる前提」の根拠（同一サイトコードS084・同一HMACキー基盤が変わらないこと）は、契約が同一条件で巻き直される想定に依存している。巻き直しでサイトコード／鍵が変わる場合はカード引き継ぎ前提そのものが揺らぐため、S-3改（前任実装からのHMACキー回収）の結果と合わせて要再確認** | システム実装とは独立した社内手続き。W-4（情シス連携）と同様に**リードタイムが長い外部依存**として早期着手が必要。9月リリース希望（Q-1）に対する重大リスク。担当・期限を決定者へ確認要 |

---

## 8. 未決定事項（`../development-plan.md` §8 ＝ 04「R5着手前チェックリスト」）

> 2026-08-19: 04-rails-implementation-plan.md の R5着手前チェックリスト（Q-25〜27・Q-35〜39）との対応を付記。**2026-08-19 v5でQ-25〜27・Q-36〜39は決定/作業前提確定済み**（Q-37〜39はネットムーブへの正式確認と並行して作業前提で実装を進める）。

| # | 論点 | 04 対応 |
|---|---|---|
| Q-7 | 支払方法の種類（クレカのみ / 口座振替等） | D-P2 で暫定（クレカ＋口振）。信販は Q-26 |
| Q-33 | 月額料金を顧客に見せない方針と checkout `item_name`/金額表示の整合 | **04 Q-33**。2026-08-18浅賀MTGで新プランは単一価格（Brige_plusのみ）と判明し、`item_name` 設計は単純化できる可能性が高い。要再確認 |
| Q-24 | 課金モデル → ✅ 継続課金で確定的 | — |
...
| ~~論点9~~ | ✅ **解消（2026-07-27設計・2026-08-19 v5 CEO最終確定）**：毎月の請求（N案件の合算含む）は**TBSS側のスコープ**となったため、`payment_transactions` は「カード登録（与信）イベント」の記録に純化される。会員IDは顧客単位（D-P10）のため、**`customer_id` を持たせる**のが自然（初回申込の案件経由で登録するため `order_id` は「登録契機の案件」として残してよい）。中間「請求」テーブルは不要 | **✅ 04 Q-36 確定**（決済トランザクションの紐づけ単位）。本書の結論＝`customer_id` + `order_id` 併記（§5-1）で確定 |
...
| 2026-08-19 | 2026-08-18 浅賀さん打ち合わせ議事録を反映。R-13 新設（ネットムーブ決済会社との契約をSSSで巻き直す必要あり。9月リリース希望に対するリスク）。§8 に Q-33 の新情報（新プラン単一価格化）を追記 |
