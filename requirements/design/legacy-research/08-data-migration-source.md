...
| DM-6 | ステータスの旧→新マッピング | 03の統廃合（受注削除・3ステータス統合）を移行時に適用 | `order_statuses` は最小セットのみ投入済み（`StatusSeeder`）。✅ **2026-08-19 v5 CEO決定**: 既定値 `0:受注` を現状維持（精査の「受注削除」案は不採用）。旧データの「受注(0)/空欄」はR7でこの初期状態へマップする |
...
> **Q-移18**（契約単位168・初期構築169 の対応先）／**DM-7**（施工担当者）／~~DM-6 の `0:受注` 既定値~~（✅2026-08-19決定・現状維持）／DM-8 の旧番号保持列。
