...
> ステータス: **ドラフト（CEO確認待ち）** — Q-A（本書 §1〜§4・§6・§7）と Q-D（§5）ともに正式決定は未了
...
**論点 Q-D-3: 分類C（`netmove_member_id` ほか決済系）** — R5 着手前に決定
...
| C-2 平文＋アクセス制御 | 分類A と同じ扱い | 追加コスト無し。カード番号は保持しない前提（ネットムーブ側保持）が崩れないことが条件 |
...
- Q-D-3 は **C-1 を推奨**（コスト小・R5 実装前なら手戻り無し）。
...
- 実データをコミットしてしまった場合: **push 前なら**直ちにコミットを取り消す
  ...
  **push 後は履歴に残るため**、秘書経由でCEOへ即報告し、履歴スクラブ（filter-repo等）と
  流出した分類B認証情報のパスワード変更依頼（発注元経由）までをワンセットで実施する。
- 外部サービスへ誤投入した場合: 投入先・範囲を記録してCEOへ報告。分類Bを含む場合は
...
| # | 条件 | 状態（2026-08-19） |
|---|---|---|
| 1 | 本ルールのCEO承認（ドラフト→確定） | ⬜ 未（本書がそのドラフト） |
| 2 | 担当者の割当（ルールの維持責任者・違反時の一次対応者） | ⬜ 未 — **（担当: ＿＿＿ ← CEO判断のプレースホルダ）** |
...
| 4 | 既存文書への反映: `development-plan.md` Q-A 行を「本書参照・確定」に更新、`release-readiness.md` C-5 / G-8 の状態欄を本書参照へ更新、`04-implementation-plan.md` R2見直しレビュー残タスク「PII方針（Q-D）」・リスク5 の消し込み | ⬜ 未 |
...
> （§5 の保留ルールで手戻りなく待てるため）。ただし **Q-D-2（分類A方針）は運用開始（R8）前、Q-D-3（分類C）は R5 着手前、
> Q-D-1（分類B移行）は R7 着手前**にそれぞれ決定が必要。
...
| 日付 | 内容 |
|---|---|
| 2026-07-26 | 初版ドラフト。3分類定義・保管/使用/移行ルール・Q-D接続・完了条件を起案（CEO確認待ち） |
