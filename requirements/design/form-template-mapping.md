...
- **2026-08-18浅賀MTGで確定（development-plan.md Q-背2）**: 新プランは**1つの価格のみ**とし、新プラン（Brige_plus）のみを販売する。プランが固定のため**プラン選択のプルダウンは不要**。`Plan` は R2 で実装済み・プラン数非依存の設計のため本節のフィールド定義には影響しないが、フォームに `plan` 選択フィールドを追加する必要は無いことの確認として記録する。なお「申込フォームを商材ごとに分けるか」は浅賀さんへの確認事項として残っており（同MTG §1）、決着すれば §1-1 の「FormTemplate は Product と 1─1」の前提と整合する
...
- SNS認証情報8カラム（`system_account_id/pass`・`google_account_id/pass`・`instagram_id/pass`・`facebook_id/pass`）は `encrypts` 対象のため
  **ホワイトリストから自動除外**され、申込フォームには載せられない（`pii-handling-rules.md` §1 分類B。R3見直しレビューでの是正）。
  > ⚠️ **2026-08-18 浅賀さん打ち合わせで新論点（development-plan.md Q-45）**: Instagram の**ID・パスワード入力を必須**にしたいという要望が出た（`mtg/2026年8月18日 浅賀さん打ち合わせ議事録.txt` §1）。これは上記の「暗号化列は動的 FormField ホワイトリストから除外し申込フォームに載せない」という現行設計方針と正面から矛盾する。動的マッピング機構を暗号化列にまで拡張するのは PII 保護意図に反するため推奨しない。実装案としては (a) クレカ登録と同様に**専用の申込ステップ（ハードコード）**として Instagram ID/PASS 入力欄を設け `Customer::Encryption` 等で暗号化保存する、(b) それでも旧 Phase 4（R6）扱いのまま契約後スタッフ入力とし「必須」は運用上の意味に留める、のいずれかを CTO 判断で選ぶ必要がある。9月リリース希望（Q-1）に対しては (a) を優先検討すべきスコープ変更。
